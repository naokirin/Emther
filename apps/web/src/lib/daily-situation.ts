...
import type { JournalEntry, OrgVitals, PersonSummary, Suggestion, VitalStatus } from "@emther/core/types";
...
function buildOrgConcernSignals(params: {
  ...
  const { now, journalEntries, vitals, people, suggestions, staleInterventionDays, push } = params;
  ...
  if (badTeams.length >= 1 || concerningTeams.length >= 2) {
    ...
    ranked.push({
      ...
      since: 0,
      onSelect: () => push(`/teams?focus=${focus.teamId}`),
  ...
  if (spreadTeams.length > 0) {
    ...
    ranked.push({
      ...
      since: 0,
      onSelect: () => push(`/people/${members[0].id}`),
      severity: 60,
    });
  }
  ...
  if (heavyPerson && heavyPerson.trend.negative >= 3) {
    ranked.push({
      ...
      since: 0,
      onSelect: () => push(`/people/${heavyPerson.id}`),
      severity: 55,
    });
  }
