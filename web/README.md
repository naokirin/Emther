# EM Support System — Agent Runtime (MVP)

`docs/first_implession/em_v5.md` で定義したアーキテクチャのうち、**3.4 CLIサブプロセス実行エンジン** と **3.6 Yield（一時停止）設計** の最小垂直スライスを実装したもの。
...
- Team Vitals（ダッシュボードの三値バイタル表示）はまだこのアプリに統合されておらず、`docs/first_implession/em_ui_wireframe_v5.html` のワイヤーフレーム止まり。
