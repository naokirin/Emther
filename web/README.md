...
- エージェントはツール利用を無効化（`--tools ""`）した「テキスト推論のみ」の存在として動作する。Organization Context（チーム名簿）をAgent Runtime側のプロンプトに読み込ませる接続はまだ無く、Lead/People/Process/Tech Agentは組織の前提を一切知らない状態で動く。
