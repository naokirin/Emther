...
- Gemini CLIフォールバック（`docs/memo.md`のTODO対応） — Settingsで有効化したエージェント種別のみ、claude CLIの実行失敗・予算/レート制限時にgemini CLIで単発ターンを再試行（実際のGemini応答成功パスはこのサンドボックスでは未検証）
...
- `/settings`に「Gemini CLIフォールバック」セクションを追加し、`AGENT_OPTIONS`（Lead/People/Process/Tech Agent）ごとにON/OFFできるチェックボックスを設置（`geminiFallbackAgents: string[]`）。
- `agent-runtime.ts`の`runClaudeTurn`を、claude呼び出し部分（`runClaudeCliAttempt`）とその結果テキスト処理（`applyAssistantResultText`、consult/yield/proposal抽出とrun状態確定の共通ロジック）に分割。`runClaudeCliAttempt`が失敗（`run.status`が`"error"`で終了、起動失敗・プロセス無応答終了・`is_error`を含む）を返し、かつそのエージェント種別でフォールバックが有効な場合のみ、`runGeminiCliAttempt`で同じターンを`gemini` CLIに再実行する。
- Gemini呼び出しは`-o text`（プレーンテキスト出力）と`--approval-mode plan`（読み取り専用、claudeの`--tools ""`相当の安全性）を使用。`--append-system-prompt`に相当するフラグが見当たらなかったため、システムプロンプトをプロンプト本文の先頭に連結して渡す。
- **既知の制約（実装時点で未検証・未対応）**: このサンドボックスにはGemini APIの認証情報が無く、実際の応答成功パスは検証できていない。また`gemini --resume`はClaudeの`--resume <uuid>`と異なりインデックス/「latest」指定のみでUUID指定に対応していないため、**Geminiフォールバックはセッション継続を持たない単発ターンとしてのみ動作する**——複数ターンの壁打ち途中でフォールバックが起きると、それまでの会話文脈は失われる。予算上限（`--max-budget-usd`相当）もGemini側には設定していない。
- 実機検証: `PER_TURN_BUDGET_USD`を一時的に極小値に変更し、claude CLIを実際に予算超過（`subtype: "error_max_budget_usd"`）で失敗させた上で、(1) `geminiFallbackAgents`が空（既定）の場合はフォールバックが一切試行されずそのまま`error`になること、(2) 対象エージェントを有効化すると実際に`gemini`バイナリが起動され、その本物のエラー（このサンドボックスでは認証未設定によるエラー）がログに記録されることを確認——実プロセスの起動・引数の受け渡しが正しく機能している証拠。設定を既定値に戻した後、通常のタスクが従来通り`idle`まで正常完了する（リファクタによる回帰が無い）ことも確認した。
...
- Gemini CLIフォールバックは、実際にGeminiが応答に成功するパス自体をこのサンドボックス（認証未設定）では検証できていない。セッション継続にも対応しておらず、複数ターンの壁打ち途中でフォールバックが起きると会話文脈が失われる。予算上限もGemini側には無い。
