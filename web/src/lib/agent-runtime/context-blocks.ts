...
export function buildSystemPrompt(
  ...
  const base = [
    ...
    '  "advice": "この提案を計画・進行・検証するうえで漏らさないほうがよい実務的なポイント（任意。結論の繰り返しではなく一歩踏み込んだ助言のときだけ）"',
    "}",
    "```",
    ...
    "- adviceは提案として残す（起票する）ときにとくに有効です。計画の立て方・進め方・検証方法・見落としがちな観点など、結論とロジックだけでは伝わらない実務的な助言があれば書いてください。特に無ければ省略してよい（無理に埋めないこと）。",
    ...issueNoteRule,
