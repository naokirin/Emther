...
// docs/2nd_architecture/plan.md フェーズ2.5: settings/data/backupはapps/server側へ移植済み
// （テストもapps/server/src/routes/settings-data-backup.test.tsへ移設）。
// reset/restoreはscheduleProcessExit()（process.exit）を呼ぶため、Next↔Hono並走期間は
// あえてNext側に残置している（plan.md参照）。
