...
- ユーザーは **どの列を出すか** と **列順** だけ指定できる（↑↓ ボタンまたはドラッグ）
...
- **A**: 提案詳細の「エクスポート」メニュー内に「Markdown をコピー」「Markdown を保存」
- **B / C**: 提案一覧は適用中フィルタ行の右端にミュートな「エクスポート」。ポップオーバー内で範囲・表コピー（TSV 主）・他形式・列設定（ドリルイン）
- 一覧の常時面に形式ボタン列や列エディタを置かない（docs/design/suggestion/suggestion-tab.pen エクスポート改善）
