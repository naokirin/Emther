...
| U19 | P0 | 完了 | 関連Issue上位5件注入のみで「無い」を確証できず、エージェントが追加照会できない |
| U20 | P2 | 完了 | 採用済みテーマが「次の1手」に訂正操作だけ並び、良いときの導線が分からない |
...
**今後必須（C）:** Claude / agy / cursor のすべてが `…/emther/data`（マスク済み業務データ）を読み取れるようにする。`secure`（実名対応表）は引き続き除外。A の合成シグナル注入は C 実現後も残す。→ **U19 でアプリ側 lookup（読み取り専用）として第一歩を実装。生のディレクトリ／SQLite を CLI に渡す方式は引き続き検討余地あり。**
...
## U19. 関連Issueの「不在」を確証できない（上位5件注入のみ）

**現象:** Issue 壁打ち（Copilot Workspace）等で「〇〇に関連するIssueが無いのは意図的か」と聞くと、エージェントが「類似度の近い上位5件しか注入されていないので分からない」と返す。EM支援として「無い」を検証できない。

**原因:**
1. `related-context.ts` がベクトル類似の上位5件・閾値0.4のみをシステムプロンプトへ注入し、ヒット0件のときはブロック自体を省略していた（沈黙＝不在の明示ではない）。
2. Claude / agy / cursor は CLI ネイティブツール無効／空 sandbox のため、SQLite や `emther/data` を自発照会できない（U14-C 未達）。

**方針:**
1. **アプリ側の読み取り専用照会（lookup）**: CLI ツールは無効のまま、エージェントが ` ```lookup``` ` ブロックで Issue キーワード検索（done/archived 込み可）・Issue ID 詳細・Journal 検索・類似検索を要求できる。ランタイムがストア／SQLite を読み、マスク済み結果を次ターンへ返す（consult と同型）。`secure` は触らない。同一 run で最大2回・1回あたり最大3クエリ。
2. **関連束の0件明示**: 注入ブロックは空にせず「閾値以上なし／必要なら lookup」と書く。
3. プロンプトを「上位5件打ち切り外は lookup で確認せよ。lookup 結果で無しなら断言してよい」に更新。

**対応:** `agent-knowledge-tools.ts` / `agent-runtime.ts`（lookup オーケストレーション）/ `related-context.ts`（0件明示）。

---

## U20. 採用済みテーマが「次の1手」で迷子になる

**現象:** 今日タブの「次の1手」内に「採用済みのテーマ解釈」が載り、操作は「編集して訂正」「壁打ちで見直す」「採用を取り消す」だけ。良いと判断したときに何をすればよいか分からない。

**原因:** 採用＝壁打ち前提への注入、という設計では「良いなら何もしない」が正解だが、判断待ちヒーローに訂正系ボタンだけが並ぶため肯定導線が読めない。

**方針:** 採用は1段階の肯定とする。採用済みは「次の1手」から外し、Dashboard に「現在の優先テーマ」セクションを新設（タイトル＋要約は常時、根拠・訂正操作は既定折りたたみ。表示は最大3件＋他）。

**対応:** `web/src/app/page.tsx`（セクション移設・折りたたみ）/ `RunDetail.tsx`（採用時コピー）/ `docs/knowledge_distillation.md`。
...
- **U19** エージェントが `lookup` で Issue/Journal を追加照会可能に。関連束は0件でも不在を明示。CLIネイティブツールは無効のまま（U14-C の安全な第一歩）
- **U20** 採用済みテーマを「次の1手」から外し、今日タブ「現在の優先テーマ」（既定折りたたみ・最大3件）へ移す。採用＝肯定の1段階
