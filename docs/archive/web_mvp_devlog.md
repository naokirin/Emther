# Emther — MVP（アーカイブ: 旧 `web/README.md`）

> **これは歴史的記録です。** 旧 Next.js 実装（`web/`）の開発ログで、フェーズ5.1（`docs/2nd_architecture/plan.md`）での `web/` 削除に伴いここへ移設した。本文中のファイルパス（`web/src/lib/...`等）は**当時のNext.js実装のもの**であり、現在の実装（`packages/core` / `apps/server` / `apps/web`、`docs/2nd_architecture.md`参照）には対応しない。セキュリティ設計（実名マスキング、people-directoryの配置分離、cursor-agentの絶対パス読み取りリスク等）の**考え方自体は現在の実装にも引き継がれている**ため、経緯の参照先として残す。内容は当時のまま変更していない。
...
- 雑多な一言メモを入力してSubmitすると、**完全にローカルで動く**軽量モデル（Transformers.js + `onnx-community/LFM2.5-350M-ONNX`, q4量子化）が構造化抽出（タグ／登場人物／緊急度／感情／要約）を行い、その場でカード表示する。
...
- モデルサイズはこのリポジトリの検証環境（メモリ7.7GB、常時スワップ逼迫気味）での安定性を優先して小型モデルを選んでいる。当初は `onnx-community/Qwen2.5-0.5B-Instruct` だったが、要約などが中国語に寄りやすかったため、日本語を含む多言語対応の `LFM2.5-350M` へ切り替えた（同規模の日本語特化 Sarashina2.2 の onnx-community 版は tokenizer 欠落で Transformers.js から使えなかった）。1.5B級は人物抽出やJSON整形の精度は高いが、この環境では2026-09-08の再検証でも1リクエストで`next-server`のRSSが約5GBまで増加してシステム空きメモリが150MB台まで低下し、生成自体もJSON抽出失敗（500）に終わったため見送っている。メモリに余裕のある環境で動かす場合は **設定 → AIツール → ローカルAI** でプリセットを選べる（既定は `350M`。日本語寄りは `1.2B` / `1.2B JP` の LFM2.5、Qwen の `0.5B` / `1.5B` も選択可。Sarashina は tokenizer 欠落のため未収録）。
- 小型モデルなりの精度限界があり、要約文が不自然になったり、人物とチーム名を混同する（例:「Bチーム」をpeopleに含めてしまう）ことがある。tags/people/urgency/sentimentは概ね実用的だが、summaryの信頼度は低めに見ておくこと。
...
- ローカルモデルの初回ロードは重み（350M・q4で数百MB）のダウンロード＋ONNX Runtime初期化を含み、リクエストが数十秒ブロックする。2回目以降はプロセス内キャッシュにより数秒程度。
