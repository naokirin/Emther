# 開発時のハイブリッド構成 並走ルール（アーカイブ）

作成日: 2026-09-19（フェーズ2.4、0.5のドラフト方針を文書化）
関連文書: `docs/2nd_architecture.md`（方針）/ `docs/2nd_architecture/plan.md`（詳細計画。6.2節に開発体験の見立て）

> **フェーズ5.1（2026-09-20、`web/` 削除）でハイブリッド並走期間そのものが終了した。** `next dev` は存在せず、通常の開発は `npm run dev -w @emther/server` + `npm run dev -w @emther/web` の2プロセス構成のみ（README.md「開発者向け」参照）。本ドキュメントは並走期間中の判断ルールの**歴史的記録**として残す。
