...
### 6.1 埋め込みモデル dtype（q8）の固定判断（2026-09-13）

実装: `web/src/lib/embeddings.ts`  
モデル: `Xenova/paraphrase-multilingual-MiniLM-L12-v2` / `dtype: "q8"`（重み量子化。出力ベクトルは正規化済み Float32）

運用開始後の dtype / モデル変更は全再埋め込みが必要なため、開始前に **同一モデルの q8 vs fp32** を日本語の EM ドメイン寄りテキストで比較した。

条件の概要:

- ラベル付きペア 20 組（related / unrelated 各 10）
- 検索: クエリ 8 × コーパス 18（Journal / Issue 風）
- 閾値 0.4・top-5 を現行関連束と同じ前提で評価
- モデルサイズ: q8 約 113MB / fp32 約 449MB

| 指標 | 結果 |
|---|---|
| 同一テキストの q8↔fp32 cosine | 平均 0.993 / 最小 0.971 |
| ラベル付きペアの閾値 0.4 判定一致 | 20/20（100%） |
| 検索の閾値判定一致 | 141/144（97.9%） |
| top-5 Jaccard 平均 | 0.875 |
| Spearman 順位相関 | 0.970 |
| \|Δcosine\| | 平均 0.015 / p95 0.036 / max 0.061 |
| related−unrelated ギャップ | q8 0.337 / fp32 0.340 |

**結論: `dtype: "q8"` のまま固定してよい。** 閾値をまたぐラベル判定の食い違いは無く、検索の不一致も僅少。fp32 への変更は品質メリットが薄い一方、サイズ増と将来の再埋め込みコストが大きい。

補足（dtype 以外）:

- 一部の言い換え「関連」ペアは **q8 / fp32 の両方で 0.4 未満**（例: 0.28–0.34）。量子化ではなくモデル＋閾値の recall 限界。
- README 相当の疲労系ペア（約 0.75）は両 dtype で問題なく閾値超え。
- チャット生成の q4 とは別判断。埋め込みを q4 に下げる検証はしていない（非推奨）。
