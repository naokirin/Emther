...
| Phase D | 完了（D1–D3＋横展開） | D1: glossary。D2: em-growth / policies / settings。D3: `json-document` 共通化。続き: org-context（backgrounds / goals / strategy / teams）・theme・em-self も同パターン。公開 `*-store` deep import は維持。未着手（複雑）: people-directory / knowledge-store / suggestion-store / journal-store / agent-runtime/store / observation-dump。 |
