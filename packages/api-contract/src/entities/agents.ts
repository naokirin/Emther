...
// toRunView が返す AgentRun。必須フィールドを厳密に、深い optional は looseObject で許容。
// HTTP ビュー型は core AgentRun と同一（ドリフト防止）。
export const agentRunViewSchema: z.ZodType<AgentRun> = z.looseObject({
  id: z.string(),
  agentName: z.string(),
  task: z.string(),
  ...
  reviewed: z.boolean(),
}) as z.ZodType<AgentRun>;

export type AgentRunView = AgentRun;

export const pendingAgentStartSchema: z.ZodType<PendingAgentStart> = z.looseObject({
  id: z.string(),
  kind: z.literal("suggestion-update"),
  label: z.string(),
  firesAt: z.number(),
}) as z.ZodType<PendingAgentStart>;

export const pendingUnmaskedSendSchema: z.ZodType<PendingUnmaskedSend> = z.looseObject({
  id: z.string(),
  kind: z.enum(["start-run", "decide-run"]),
  candidates: z.array(z.string()),
  label: z.string(),
}) as z.ZodType<PendingUnmaskedSend>;
