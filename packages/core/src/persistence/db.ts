...
// 同じ`.data/app.db`へマイグレーションを試みることがある。事前にcolumnExistsで
...
// Issue→Suggestion統合の命名統一。旧カラム名が残る既存DBだけ
...
function migrate(database: DatabaseSync): void {
  ...
  // 人物についてのイベント（peopleで名前を持つ）だけでなく、
  ...
  // AIが提案するWhy/What/Howの下書き（未整理の項目のみ埋める提案。既存の
  ...
  // 週次・月次レビューの構造化出力（PeriodReview）と、材料となったreports行への逆リンク。
  ...
  // 生成結果（stats_json）自体をスナップショットとして保存する（その場の再計算だけにしない）。
  // 後から状態が変わるデータを元に算出しているため、「生成時点でEMに何が見えていたか」を
  // 再計算せず保持することを優先。noteはEMが後から追記できる所感欄。
  database.exec(`
  ...
  // AIやヒューリスティックが検出した状態自体は観測事実として書き換えず、
  // 「EMが確認し、対応不要と判断した」上書きだけを別途持たせ、UIはこれがあれば強調を弱める。
  // 3箇所で同じカラム名ペア（no_action_needed_at / no_action_needed_note）を使う。
