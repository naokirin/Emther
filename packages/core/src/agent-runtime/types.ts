...
// Expand/Challengeの過程で実際に使った哲学レンズ（Agile/Lean/
...
// EMが相談で「提案を整理して」等と明示的に
// したときだけ、AIが既存提案（実在ID）の状態変更をまとめて提案する。EMが「まとめて
...
// 週次・月次レビューの構造化出力。事実・解釈・提案を分離する方針に沿い、
// Proposal（conclusion/facts/logic）とは別に、期間レビュー特有の観点（Before/After・
...
export type PeriodReview = {
  ...
  learnings: string[];
  // 来週/来月考えたい問い。次回同originのレビュー起動時にbuildPeriodReviewContextBlock
  // が前回runのこの値を材料へ注入し、問いのループを実装する。
...
export type AgentRun = {
  ...
  // 週次・月次レビューの構造化出力そのもの。proposalと違い「採用するまで反映しない下書き」ではなく、
  // このrunの主出力（EMは会話継続で応答するのみ。HITLの対象は同時に出せるthemesの方）。
  periodReview?: PeriodReview;
  totalCostUsd: number;
  createdAt: number;
  updatedAt: number;
  // Lead Agentが専門エージェントに相談した際、相談先のrunにはconsultedBy（相談元Lead run id）を付与し、
  // EM向け表示で「誰から相談されたrunか」を追跡できるようにする。pendingConsultは
  // handleStreamEventからrunClaudeTurnへ「相談したい」を伝える一時フィールドで、
  // 外部からは基本的に参照しない。
  consultedBy?: string;
  pendingConsult?: ConsultRequest;
  // アプリ側の読み取り専用照会（```lookup```）。pendingLookupは
  ...
  // 週次・月次レビューは"auto-weekly-report"/"auto-monthly-report"。
