import {
  createJsonJournalBatchRepository,
  type JournalBatchRepository,
} from "../persistence/adapters/json-journal-batch-repository";
import type { JournalBatchPersisted } from "./journal-batch-types";

export type { JournalBatchPersisted } from "./journal-batch-types";
...
let repo: JournalBatchRepository = createJsonJournalBatchRepository();

/** テスト用: リポジトリ差し替え。 */
export function setJournalBatchRepositoryForTest(next: JournalBatchRepository): void {
  repo = next;
}
