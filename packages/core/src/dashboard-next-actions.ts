...
import { truncateExcerpt } from "./origin-trace";
...
export type NextActionTarget =
  ...
  | { type: "run-suggestion"; runId: string }
  | { type: "prefill-journal"; text: string }
  | { type: "confirm-unmasked"; pendingId: string };
...
/** heroRank / rankActions / urgencyMeter が参照するフィールド（UI View 型とも共有）。 */
export type NextActionRankFields = Pick<NextAction, "id" | "severity" | "lane" | "kindLabel" | "since">;

/** ダッシュボードの「次の1手」優先度。本物の判断ブロッカー → 起票待ちドラフト → その他判断 → 観測 → 整備。 */
export function heroRank(a: NextActionRankFields): number {
...
function draftTarget(run: AgentRun, suggestions: Suggestion[]): NextActionTarget {
  if (run.origin === "auto-suggestion-update") {
    const linked = suggestions.find((s) => s.agentRunId === run.id);
    if (linked) return { type: "path", path: `/suggestions/${linked.id}` };
  }
  return { type: "path", path: `/chat?runId=${run.id}` };
}
...
function runActionTarget(
  ...
  suggestions: Suggestion[],
  isDraft: boolean,
  isLeadUnlinked: boolean,
): NextActionTarget {
  if (isDraft || isLeadUnlinked) return draftTarget(run, suggestions);
  return { type: "run-suggestion", runId: run.id };
}
...
export function buildNextActions(params: BuildNextActionsParams): NextAction[] {
  ...
  for (const run of runs) {
    ...
    const selectTarget = runActionTarget(run, suggestions, isDraft, isLeadUnlinked);
    ...
    if (staleRunIds.has(run.id)) {
      ...
      nextActions.push({
        ...
        target: draftTarget(run, suggestions),
        since: run.updatedAt,
        ctaLabel: "進捗を見る",
      });
    ...
    if (v.status === "unknown") {
      ...
      nextActions.push({
        ...
        target: {
          ...
          text: v.members.length > 0 ? `#1on1 @${v.members[0]} ` : "",
        },
        since: 0,
      });
    }
  ...
  if (vitals.oneOnOneCoverage.status === "bad" || vitals.oneOnOneCoverage.status === "warn") {
    nextActions.push({
      ...
      target: {
        type: "prefill-journal",
        text: vitals.oneOnOneCoverage.uncoveredMembers[0]
          ? `#1on1 @${vitals.oneOnOneCoverage.uncoveredMembers[0]} `
          : "",
      },
      since: 0,
    });
  ...
  for (const run of watchingItems) {
    ...
    nextActions.push({
      ...
      target: { type: "path", path: `/chat?runId=${run.id}` },
  ...
  for (const pending of pendingAgentStarts) {
    nextActions.push({
      ...
      // suggestionId が無いときはクリックで何もしない（空 path）。
      target: {
        type: "path",
        path: pending.suggestionId ? `/suggestions/${pending.suggestionId}` : "",
      },
  ...
  for (const pending of pendingUnmaskedSends) {
    nextActions.push({
      ...
      target: { type: "confirm-unmasked", pendingId: pending.id },
      since: now,
    });
  }
